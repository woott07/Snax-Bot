...
module.exports = {
    ...
    execute(message, args) {
        if (!args[0]) return message.reply('❌ Please mention a role or user. Example: `$remNickExe @Moderator`');
        ...
        if (removed) {
            message.reply(`✅ Revoked **setNickExe** permissions from ${target}`);
        } else {
            message.reply(`⚠️ ${target} does not have setNickExe permissions.`);
        }
    }
};
